...
class STSGCM(nn.Module):
    ...
    def forward(self, x, mask=None):
        """
        :param x: (3N, B, Cin)   不是（4N，B，Cin）吗？
        :param mask: (3N, 3N)
        :return: (N, B, Cout)
        """
        need_concat = []
...
class STFGNN(nn.Module):
    def __init__(self, config, data_feature):
        ...
        self.adj = adj
        self.num_of_vertices = num_of_vertices
        self.hidden_dims = hidden_dims  #"hidden_dims": [[64, 64, 64], [64, 64, 64], [64, 64, 64]],
        ...
        for idx, hidden_list in enumerate(self.hidden_dims):  # self.hidden_dims="hidden_dims": [[64, 64, 64], [64, 64, 64], [64, 64, 64]],
            #print("?????? ", idx)
            if idx == 0:
                continue
            #print("---------", idx)
    ...
    def forward(self, x):
        ...
        #print(2)
        need_concat = []
        for i in range(self.horizon): # horizon=12
