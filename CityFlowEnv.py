...
class TrafficSignal:
    ...
    def __init__(self, ID, eng, yellow_time, roadnet_info):
        ...
        self.set_eng_phase()
        self.green_wave_action_space=10
    ...
    def _set_observation_space(self):
        ...
        self.observation_space = {
            ...
            'TSphase': self.roadnet_info['phases'],
            'TStime': [1],
            'Envtime': [1],
    ...
    def get_state(self):
        ...
        for i,roadlink in enumerate(self.roadnet_info['roadlinks']):
            count = self.eng.get_roadlink_vehicle_count(self.ID, i)
...
class Intersection:
    def __init__(self, ID, config, eng, intersection_names, 
        ...
        self._set_green_wave_action_space()
    ...
    def _init_components(self):
        ...
        self.saved_12phases=np.zeros((12,), dtype = int)
        self.saved_12phases[:] = -1
        ...
        self.lane_vehicle_out_count_mat = np.zeros((
            self.save_lane_count_step, 
            len(self.roadnet_info['connection']), 3), dtype = int)
        self.TSstfflow=np.zeros((12,12),dtype = int)
    ...
    def _set_observation_space(self):
        ...
        tot['TSprev12phases'] = [12]

        i=0
        RoadLength=[]
        while i<12:
             road=self.roadnet_info['roadlinks'][i]['startRoad'].split('_')
             road=f'{road[0]}_{road[2]}_{road[1]}'
             RoadLength.append(float(self.roadnet_info['connection'][road][1]))
             i+=3
        tot['RoadLength']=RoadLength
    ...
    def _set_green_wave_action_space(self):
        tot = [self.TS.green_wave_action_space]
        self.green_wave_action_space = tot
    ...
    def get_state(self):
        ...
        DCphase = []
        self.TSstfflow[:,-1]=np.array(res['TSflow']).T
        ...
        self.saved_12phases[-1] = res['TSphase']
        res['TSprevphases'] = list(self.saved_phases)
        res['TSprev12phases'] = list(self.saved_12phases)
        res['DCphase'] = DCphase
        res['pressure'] = self._get_pressure_observation()
        res['TSstfflow']=self.TSstfflow
        return res
    ...
    def green_wave_act(self, actions, pattern):
        if len(actions) == 1:  # input single number, treat as combined action
            arr = []
            action = actions[0]
            for i in self.green_wave_action_space:
                arr.append(action % i)
                action //= i
            actions = arr
        self.set_speed(actions[0], pattern)
        assert len(actions) == 1
...
class CityFlowEnv:
    ...
    def _set_green_wave_action_space_one(self, i):
        return self.list_intersection[i].green_wave_action_space

    def _set_green_wave_action_space(self):
        self.green_wave_action_space = []
        for i in range(len(self.list_intersection)):
            self.green_wave_action_space.append(self._set_green_wave_action_space_one(i))
    ...
    def step_lane_vehicles(self):
        for inter in self.list_intersection:
            ...
            inter.saved_12phases[:-1] = inter.saved_12phases[1:]
            inter.TSgcnflow[:,:-1]=inter.TSgcnflow[:,1:]
            inter.TSgcnflow[:,-1]=0
