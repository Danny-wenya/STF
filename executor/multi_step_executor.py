...
class MultiStepExecutor(object):
    ...
    def train(self, train_data, valid_data):
        ...
        # for epoch in tqdm.tqdm(range(1, self.epochs + 1)): #进度条
        for epoch in range(1, self.epochs + 1):
            ...
            self._writer.add_scalar(f"train MSE loss", loss,epoch)
            self._writer.add_scalar(f"valid MSE loss", loss,epoch)

            print(
                '| end of epoch {:3d} | time: {:5.2f}s | train_loss {:5.4f} | valid mse {:5.4f}'.format(
